<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('postgroups.update', $postGroup) }}">
            @csrf
            @method('patch')
            <!-- Group Name -->
            <div>
                <x-input-label for="name" :value="__('Name')" />
                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $postGroup->name)" required autofocus autocomplete="name" />
            ...
            <div class="mt-4">
                ...
                <x-text-input id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description', $postgroup->description)" required />
