...
use Illuminate\Auth\Events\CreateNewPasswordLinkSent;
...
class PasswordBroker implements PasswordBrokerContract
{
     ...
     * @return string
     */
    public function sendCreateNewLink(#[\SensitiveParameter] array $credentials, ?Closure $callback = null)
    {   
        // First we will check to see if we found a user at the given credentials and
        // if we did not we will redirect back to this current URI with a piece of
        // "flash" data in the session to indicate to the developers the errors.
        ...
        if ($callback) {
            return $callback($user, $token) ?? static::CREATE_NEW_LINK_SENT;
        }

        // Once we have the reset token, we are ready to send the message out to this
        // user with a link to reset their password. We will then redirect back to
        // the current URI having nothing set in the session to indicate errors.
        $user->sendCreateNewPasswordNotification($token);

        $this->events?->dispatch(new CreateNewPasswordLinkSent($user));

        return static::CREATE_NEW_LINK_SENT;
    }
     ...
     * @return mixed
     */
    public function reset(#[\SensitiveParameter] array $credentials, Closure $callback, string $status = 'RESET')
    {
